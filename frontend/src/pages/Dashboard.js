...
const Dashboard = () => {
    ...
    // This is a simple counter. When we want to refetch data, we just increment it.
    // This is the most reliable way to trigger the useEffect hook.
    const [refetchCounter, setRefetchCounter] = useState(0);

    useEffect(() => {
        setLoading(true);
        // This effect will run on the initial page load, when the search term changes,
    ...
    // Guard clause: Prevents the rest of the component from rendering until the user object is loaded.
    // This stops crashes related to checking 'user.role' when 'user' is still null.
    if (!user) {
        return (
