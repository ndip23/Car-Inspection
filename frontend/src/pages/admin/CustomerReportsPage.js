    ...
    <ReportTable
        ...
        rows={data.map((item, index) => (
            <tr key={item.vehicleId} className="border-b border-light-border/50 dark:border-dark-border/50">
                ...
                <td className="p-3 font-semibold text-center">{item.inspectionCount}</td>
            </tr>
        ))}
...
const CustomerReportsPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            setReportData([]); // Clear previous data to prevent showing old data while loading
            try {
                const response = activeTab === 'loyal' 
                    ? await fetchLoyalCustomers() 
                    : await fetchLapsedCustomers();
                
                // Safety check: ensure response data is an array before setting state
                setReportData(Array.isArray(response.data) ? response.data : []);
            } catch (error) {
                toast.error('Failed to load report data.');
                setReportData([]); // Set to empty array on error to prevent any crash
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex border-b border-light-border dark:border-dark-border">
                <TabButton name="loyal" activeTab={activeTab} setActiveTab={setActiveTab} icon={<FiStar />}>Loyal Customers</Tab-Button>
                <TabButton name="lapsed" activeTab={activeTab} setActiveTab={setActiveTab} icon={<FiUsers />}>Lapsed Customers</Tab-Button>
            </div>
            ...
            <div className="p-6 rounded-xl glass-card">
                {loading ? (
                    ...
                    // This conditional rendering is safe because reportData is guaranteed to be an array
                    activeTab === 'loyal' 
                        ? <LoyalCustomersTable data={reportData} /> 
                        : <LapsedCustomersTable data={reportData} />
                )}
            </div>
        </div>
    );
